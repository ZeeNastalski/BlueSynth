class InputController {
    constructor(voiceManager) {
        ...
        console.log("InputController: Instance created.");
    }

    initializeInputs() {
        console.log("InputController: Initializing inputs...");
        try {
            this.initMIDI();
            this.setupKeyboardListeners();
            this.updateOctaveDisplay();
            this.updateMIDIStatus();
            console.log("InputController: Inputs initialized successfully.");
        } catch (error) {
            console.error("InputController: Error during input initialization:", error);
        }
    }
    ...
    async initMIDI() {
        console.log("InputController: initMIDI called.");
        if (!navigator.requestMIDIAccess) {
            console.error('InputController: WebMIDI API not available.');
            this.updateMIDIStatus();
            return;
        }

        try {
            console.log("InputController: Requesting MIDI access...");
            this.midiAccess = await navigator.requestMIDIAccess();
            console.log("InputController: MIDI access granted.", this.midiAccess);
            ...
            console.error('InputController: Error accessing MIDI devices:', error);
            this.midiAccess = null;
            this.updateMIDIStatus();
        }
    }
    ...
    handleMIDIMessage(message) {
        ...
        console.log(`InputController: MIDI Message - Cmd: ${command}, Note: ${note}, Vel: ${velocity}`);
    ...
    setupKeyboardListeners() {
        console.log("InputController: setupKeyboardListeners called.");
        const keys = document.querySelectorAll('.key');
        console.log(`InputController: Found ${keys.length} key elements for virtual keyboard.`);
        if (keys.length === 0) {
        ...
        keys.forEach(key => {
            ...
            const handleMouseDown = (e) => {
                e.preventDefault();
                console.log(`InputController: MouseDown/TouchStart on key ${note}`);
            ...
            const handleMouseUpOrLeave = () => {
                if (key.classList.contains('active')) {
                    console.log(`InputController: MouseUp/TouchEnd/MouseLeave detected for key ${note}`);
                    if (note) {
                        try {
                            this.voiceManager.stopNote(note);
        ...
        console.log("InputController: Setting up physical keyboard listeners.");
        document.addEventListener('keydown', (e) => {
            ...
            if (note && !this.pressedKeys.has(e.code)) {
                e.preventDefault();
                console.log(`InputController: KeyDown '${e.key}' mapped to note ${note}`);
        ...
        document.addEventListener('keyup', (e) => {
            ...
            if (note && this.pressedKeys.has(e.code)) {
                e.preventDefault();
                console.log(`InputController: KeyUp '${e.key}' mapped to note ${note}`);
        ...
        console.log("InputController: Keyboard listeners setup complete.");
    }
